...
class AnalyticsService:
    ...
    def get_sensor_data_last_8_hours(self, sensor_id: int, hours: int = 8, limit: int = 16):
        client = self._get_client()
        try:
            query = f"""
            SELECT time, data_temperature, data_humidity, source_address, sensor_id, topic
            ...
            return formatted_records, None

        except Exception as e:
            return None, str(e)
        finally:
            client.close()
    def get_sensor_data_last_month(self, sensor_id: int, days: int = 30):
        client = self._get_client()
        try:
            query = f"""
            SELECT time, data_temperature, data_humidity, source_address, sensor_id, topic
