...
def get_workspaces(user, request):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({'error': 'Database Error', 'message': 'Unable to retrieve workspaces'}), 500
...
def get_workspace(workspace_id):
    """Récupère un workspace spécifique avec gestion d'erreur robuste"""
    try:
        workspace = Workspace.query.get(workspace_id)
        
        if not workspace:
            return jsonify({'error': 'Not Found', 'message': f'Workspace with ID {workspace_id} not found'}), 404
        ...
        db.session.rollback()
        return jsonify({'error': 'Database Error', 'message': 'Unable to retrieve workspace'}), 500
...
def create_workspace():
    ...
    try:
        ...
        return jsonify({'message': 'Workspace created successfully', 'id': workspace.id}), 201
        
    except IntegrityError as e:
        logger.error(f"Erreur d'intégrité lors de la création du workspace: {str(e)}")
        db.session.rollback()
        return jsonify({'error': 'Validation Error', 'message': 'Invalid data for workspace creation'}), 400
        
    except SQLAlchemyError as e:
        logger.error(f"Erreur de base de données lors de la création du workspace: {str(e)}")
        db.session.rollback()
        return jsonify({'error': 'Database Error', 'message': 'Unable to create workspace'}), 500
...
def update_workspace(workspace_id):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({'error': 'Validation Error', 'message': 'Invalid data for workspace update'}), 400
        
    except SQLAlchemyError as e:
        logger.error(f"Erreur de base de données lors de la mise à jour du workspace {workspace_id}: {str(e)}")
        db.session.rollback()
        return jsonify({'error': 'Database Error', 'message': 'Unable to update workspace'}), 500
...
def delete_workspace(workspace_id):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({'error': 'Database Error', 'message': 'Unable to delete workspace'}), 500
